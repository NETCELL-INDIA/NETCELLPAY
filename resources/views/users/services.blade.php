        ...
        <a href="{{ url($svc['route']) }}" class="text-decoration-none text-dark">
            <div class="card h-100 shadow-sm border-0">
                <div class="card-body text-center py-4">
                    <img class="rounded-circle mb-3" src="{{ $svc['icon_url'] ?? URL::asset($svc['icon']) }}" alt="{{ $svc['name'] }}" style="width:72px;height:72px;object-fit:cover" onerror="this.src='{{ URL::asset('service_icon/mobile_1.png') }}'">
...
<div class="row g-3 mt-2">
    @foreach(\helpers::serviceCatalogItems('bbps') as $cat)
        ...
        <a href="{{ url('users/services/bill-payments?id=' . $cat['id'] . '&name=' . urlencode($cat['name'])) }}" class="text-decoration-none text-dark">
            <div class="card h-100 border">
                <div class="card-body text-center p-3">
                    <img src="{{ $cat['logo_url'] ?? URL::asset($cat['logo']) }}" alt="{{ $cat['name'] }}" style="height:48px" onerror="this.src='{{ URL::asset('service_logo/10.png') }}'">

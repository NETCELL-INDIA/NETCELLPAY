...
use Session;
class StateWizeSwitchController extends Controller
{
    public function index(Request $post)
    {
        return view('admin.system.state-wize-switch');
    }
    ...
    public function fetchAll(Request $post)
    {
        ...
        $start= ($page-1) * $limit;
        $total_row = DB::table('state_wize_switch')->get();
        ...
        $list = DB::table('state_wize_switch')->orderBy('id', 'DESC')->offset($start)->limit($limit)->get();
    ...
    {
        $delete = DB::table('state_wize_switch')->where('id', '=',$post->id)->delete();
        if($delete){
            $data['type'] = 'success';
    ...
    {
        $get = DB::table('state_wize_switch')
        ->join('providers','providers.id' , '=', 'state_wize_switch.provider_id')
        ->where('state_wize_switch.id', $post->id)
        ->first([
            'providers.provider_name',
            'state_wize_switch.id',
            'state_wize_switch.user_id',
            'state_wize_switch.provider_id',
            'state_wize_switch.service_id',
            'state_wize_switch.amount',
            'state_wize_switch.api_id',
            'state_wize_switch.state_id',
            'state_wize_switch.status',
            'state_wize_switch.created_at',
            'state_wize_switch.updated_at',
        ]);
        if($get){
            $data['type'] = 'success';
    ...
    public function updateData(Request $post)
    {
       ...
       // return $post->account_type;
        if($post->edit_id==0){
            ...
            $exit = DB::table('state_wize_switch')->where('service_id',$post->service_id)->where('provider_id',$post->provider_id)->where('state_id',$post->state_id)->first();
            if($exit){
                return response()->json(array(
                    'type' => 'error',  
            ...
            $update = DB::table('state_wize_switch')->insert([
                'user_id' => 1,
            ...
            $update = DB::table('state_wize_switch')->where('id', $post->edit_id)->update([
                'api_id' => $post->api_id,
                'amount' => $post->amount_switch,
                'status' => $post->status,
                'updated_at' => Carbon::now()

...
use Session;
class AmountWizeSwitchController extends Controller
{
    public function index(Request $post)
    {
        return view('admin.system.amount-wize-switch');
    }
    ...
    public function fetchAll(Request $post)
    {
        ...
        $start= ($page-1) * $limit;
        $total_row = DB::table('amount_wize_switch')->get();
        ...
        $list = DB::table('amount_wize_switch')->orderBy('id', 'DESC')->offset($start)->limit($limit)->get();
    ...
    {
        $delete = DB::table('amount_wize_switch')->where('id', '=',$post->id)->delete();
        if($delete){
            $data['type'] = 'success';
    ...
    {
        $get = DB::table('amount_wize_switch')
        ->join('providers','providers.id' , '=', 'amount_wize_switch.provider_id')
        ->where('amount_wize_switch.id', $post->id)
        ->first([
            'providers.provider_name',
            'amount_wize_switch.id',
            'amount_wize_switch.user_id',
            'amount_wize_switch.provider_id',
            'amount_wize_switch.service_id',
            'amount_wize_switch.amount',
            'amount_wize_switch.api_id',
            'amount_wize_switch.status',
            'amount_wize_switch.created_at',
            'amount_wize_switch.updated_at',
        ]);
        if($get){
            $data['type'] = 'success';
    ...
    public function updateData(Request $post)
    {
       ...
       // return $post->account_type;
        if($post->edit_id==0){
            ...
            $exit = DB::table('amount_wize_switch')->where('service_id',$post->service_id)->where('provider_id',$post->provider_id)->first();
            if($exit){
                return response()->json(array(
                    'type' => 'error',  
            ...
            $update = DB::table('amount_wize_switch')->insert([
                'user_id' => 1,
            ...
            $update = DB::table('amount_wize_switch')->where('id', $post->edit_id)->update([
                'api_id' => $post->api_id,
                'amount' => $post->amount_switch,
                'status' => $post->status,
                'updated_at' => Carbon::now()

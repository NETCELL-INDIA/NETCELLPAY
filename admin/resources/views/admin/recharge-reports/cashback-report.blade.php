...
@section('title') Cashback Report @endsection

@section('css')
<link href="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css" rel="stylesheet" />
<style>
    .select2-container { width: 100% !important; }
    .cb-summary .badge-pill-stat {
    ...
    .stat-received { color: #d39e00; border-color: #ffc107; }
    .stat-cashback { color: #dc3545; border-color: #dc3545; }
</style>
@endsection
...
<div class="row">
    <div class="col-12">
        <div class="page-title-box">
            <h4 class="mb-0">Cashback Report</h4>
        </div>
    </div>
</div>
...
<div class="card">
    ...
    <div class="card-body">
        <div class="cb-summary mb-3" id="summaryPills">
            <span class="badge-pill-stat stat-mrp">MRP : 0.00</span>
            <span class="badge-pill-stat stat-received">Received : 0.00</span>
            <span class="badge-pill-stat stat-cashback">Cashback: 0.00</span>
        </div>
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-striped align-middle mb-0">
                ...
                <tbody id="cashbackBody">
                    <tr><td colspan="8" class="text-center text-muted py-4">No data available in table</td></tr>
                </tbody>
            </table>
        </div>
...
function renderSummary(s) {
    $('#summaryPills').html(
        ...
        '<span class="badge-pill-stat stat-received">Received : ' + (s.received || '0.00') + '</span>' +
        '<span class="badge-pill-stat stat-cashback">Cashback: ' + (s.cashback || '0.00') + '</span>'
    );
}

function fetchCashback() {
    $('#cashbackBody').html('<tr><td colspan="8" class="text-center text-muted py-4">Loading...</td></tr>');
    $.ajax({
        url: '{{ route("cashbackReportList") }}',
        method: 'POST',
        dataType: 'json',
        data: filterPayload(),
        success: function (res) {
            if (!res || res.type !== 'success') {
                $('#cashbackBody').html('<tr><td colspan="8" class="text-center text-danger">Failed to load</td></tr>');
                return;
            }
            $('#cashbackBody').html(res.rows);
...
function downloadCsv() {
    var form = $('<form>', { method: 'POST', action: '{{ route("cashbackReportDownload") }}' });
...
$(function () {
    ...
    $('#btnSubmit').on('click', function () { currentPage = 1; fetchCashback(); });
    $('#btnDownload').on('click', downloadCsv);
    $('#show').on('change', function () { currentPage = 1; fetchCashback(); });
    $('#btnPrev').on('click', function () { if (currentPage > 1) { currentPage--; fetchCashback(); } });
    $('#btnNext').on('click', function () { if (currentPage < lastPage) { currentPage++; fetchCashback(); } });
